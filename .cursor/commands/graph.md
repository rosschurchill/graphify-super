...
Parse the optional path from the invocation. Examples:
- `/graph` → target = `.` (project root)
- `/graph src/` → target = `src/`
- `/graph graphify/` → target = `graphify/`
...
- This command never makes LLM API calls — it's always safe to run.
